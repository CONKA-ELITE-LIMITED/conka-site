...
- **The blog is static: Notion is read at build only.** A write to a Published row is picked up by the hourly auto-publish cron, which redeploys (see `docs/features/BLOG_SYSTEM.md`, Auto-publish). A script that edits Published rows (a topic backfill, a strip pass) therefore ships within the hour; new rows are written as `Draft` and ship nothing. Verify on the live site after the next build.
