...
| - | Money-page "From £X/shot" prices derived from `BYO_PRICING` | Removes drift between the meta descriptions and the Product JSON-LD, which already derives from the same source. | SCRUM-1139 |
...
- **Prices and discounts derive from one source.** The Product JSON-LD and the "From £X/shot" meta text both read `BYO_PRICING`, so a price change cannot leave stale figures in the index. "From" means the cheapest cadence (quarterly). Displayed discounts share that table: every "Save X%" on the site resolves through `getDisplayDiscount(pricing)`, which returns the SKU's explicit `discountPercent` (the published figure that factors in free shots) or falls back to the derived saving vs `compareAtPrice`. So on-page discounts, meta prices and schema all trace to the same source and cannot drift. See `docs/PRICING_HISTORY.md`.
...
- **Meta prices, JSON-LD and displayed discounts all come from `BYO_PRICING`.** Prices via `getByoPriceRange` / `getByoMinPerShot`; discounts via `discountPercent` + `getDisplayDiscount`. When a price OR discount changes, update `BYO_PRICING` and append a dated block to `docs/PRICING_HISTORY.md`.
...
- Google Rich Results Test on a PDP: exactly one `Product` and one `FAQPage` node, prices matching `byoData.ts`.
