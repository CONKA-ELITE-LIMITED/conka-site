...
- **Meta prices, JSON-LD and displayed discounts all come from `OFFER_PRICING`.** Prices via `getByoPriceRange` / `getByoMinPerShot`; discounts via `discountPercent` + `getDisplayDiscount`. When a price OR discount changes, update `OFFER_PRICING` and append a dated block to `docs/PRICING_HISTORY.md`.
