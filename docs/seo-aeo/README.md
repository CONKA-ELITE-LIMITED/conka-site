...
| `app/lib/funnelData.ts` | `FUNNEL_PRICING` (single source of truth for both prices and displayed discounts); `getFunnelPriceRange` feeds the JSON-LD, `getFunnelMinPerShot` feeds the meta "From" price, and `getDisplayDiscount` resolves each SKU's shown "Save X%" (explicit `discountPercent`, else derived from `compareAtPrice`). The `app/(trial-b)/lib/funnelData.ts` copy mirrors this for the noindex A/B funnel pages and must be kept in sync. |
...
- **Prices and discounts derive from one source.** The Product JSON-LD and the "From £X/shot" meta text both read `FUNNEL_PRICING`, so a price change cannot leave stale figures in the index. "From" means the cheapest cadence (quarterly). Displayed discounts share that table: every "Save X%" on the site resolves through `getDisplayDiscount(pricing)`, which returns the SKU's explicit `discountPercent` (the published figure that factors in free shots) or falls back to the derived saving vs `compareAtPrice`. So on-page discounts, meta prices and schema all trace to the same source and cannot drift. See `docs/PRICING_HISTORY.md`.
...
- **Meta prices, JSON-LD and displayed discounts all come from `FUNNEL_PRICING`.** Prices via `getFunnelPriceRange` / `getFunnelMinPerShot`; discounts via `discountPercent` + `getDisplayDiscount`. When a price OR discount changes, update `FUNNEL_PRICING` (and its `app/(trial-b)` copy) and append a dated block to `docs/PRICING_HISTORY.md`.
