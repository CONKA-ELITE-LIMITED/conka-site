...
| - | Money-page "From £X/shot" prices derived from `OFFER_PRICING` | Removes drift between the meta descriptions and the Product JSON-LD, which already derives from the same source. | SCRUM-1139 |
...
| `app/lib/offerData.ts` | `OFFER_PRICING` (single source of truth for both prices and displayed discounts); `getOfferPriceRange` feeds the JSON-LD, `getOfferMinPerShot` feeds the meta "From" price, and `getDisplayDiscount` resolves each SKU's shown "Save X%" (explicit `discountPercent`, else derived from `compareAtPrice`). (The former `app/(trial-b)/lib` fork was merged into this file in SCRUM-1247; there is no copy to keep in sync any more.) |
...
- **robots allows AI crawlers and does not block noindex pages.** AEO citation is an explicit goal, so AI crawlers stay allowed. The noindex ad/order-flow pages are not disallowed in robots, because blocking them would stop Google seeing their `noindex` tag.
- **Prices and discounts derive from one source.** The Product JSON-LD and the "From £X/shot" meta text both read `OFFER_PRICING`, so a price change cannot leave stale figures in the index. "From" means the cheapest cadence (quarterly). Displayed discounts share that table: every "Save X%" on the site resolves through `getDisplayDiscount(pricing)`, which returns the SKU's explicit `discountPercent` (the published figure that factors in free shots) or falls back to the derived saving vs `compareAtPrice`. So on-page discounts, meta prices and schema all trace to the same source and cannot drift. See `docs/PRICING_HISTORY.md`.
...
- **Meta prices, JSON-LD and displayed discounts all come from `OFFER_PRICING`.** Prices via `getOfferPriceRange` / `getOfferMinPerShot`; discounts via `discountPercent` + `getDisplayDiscount`. When a price OR discount changes, update `OFFER_PRICING` and append a dated block to `docs/PRICING_HISTORY.md`.
...
- Google Rich Results Test on a PDP: exactly one `Product` and one `FAQPage` node, prices matching `offerData.ts`.
