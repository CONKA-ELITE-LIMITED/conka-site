...
| `app/nike/TrialCalendar.tsx` | Server. Plain calendar: 3 baseline days then the Mon-Sun fortnight, opaque phase colours. |
...
- **Trial dates are hard-coded.** Kickoff Thursday 6 August; baseline Thu 6 - Sat 8 Aug; fortnight Mon 10 - Sun 23 Aug. They live in `TrialCalendar.tsx` (`BASELINE`, `CELLS`) and in the hero/kickoff copy in `page.tsx`. Change both if the schedule moves.
...
- **Calendar / dates:** `TrialCalendar.tsx` plus the hero kickoff block.
