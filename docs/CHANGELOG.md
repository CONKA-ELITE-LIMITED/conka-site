...
### 2026-05-27 -- /start perf: mirror home page import pattern

Follow-up to the Phases 1-4 perf rebuild after a fresh Lighthouse run on the merged branch showed `/start` at Perf 59-65 (median across two Slow 4G runs) versus home at Perf 72 with FCP/LCP roughly comparable but home's TBT 50ms vs `/start`'s 330-670ms. The page architectures had diverged: home (`app/page.tsx`) wraps every below-fold `dynamic()` import with an explicit `{ loading: () => <div className="h-[Xpx]" /> }` skeleton, while `/start` had a mix of static imports for the server components (introduced in Phase 3) and bare `dynamic()` calls without loading skeletons for the client islands. Rewrites `app/start/page.tsx` to mirror home one-for-one: only `Navigation`, `Footer`, and `CROHeroV2` stay as static imports; every other below-fold section becomes `dynamic(() => import("..."), { loading: () => <div className="h-[Xpx]" /> })` with skeleton heights matching the prior CROBelowFold values. Server-component imports (CROBrandStory, CROBenefitCards, CROResearch, CROAppCallout, CROFAQv2, LandingDisclaimer) are no longer statically imported. Metadata export (noindex) and V2 visual styling preserved untouched. Build clean, `/start` still `○ (Static)`. The architectural difference between the two pages is now zero; any remaining perf gap is component weight (CROBuyBox cart hooks, CROAthletes 7 portraits, CROCustomerReviews 24 cards rendered, CROFormulaSplitV2 13 ingredient rows), not page-level wiring. Lighthouse re-measurement pending after preview deploy.
