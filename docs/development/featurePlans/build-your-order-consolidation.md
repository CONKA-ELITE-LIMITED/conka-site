...
| Internal prefix | `byo` | `BYO_URL`, `offerData.ts`, `byoCheckout.ts`, events `byo:*`, cart `_source: "byo_page"`. |
...
| 3 | Copy and asset alignment (section-by-section, user-directed) | Done (merged 25 Aug 2026); Phase 4 traffic decision is next |
...
- **S2. Bob-style mobile top: sticky media gallery.** A pinned media band under the step chrome: product video plus swipeable stills (arrows + swipe, reusing the PDP gallery assets), widget scrolling beneath. Optional narrow navy offer ribbon between gallery and content, derived live from `offerData` so it cannot drift.
