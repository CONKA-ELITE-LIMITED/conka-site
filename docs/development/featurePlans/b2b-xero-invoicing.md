...
- **VAT handling for B2B is a separate workstream (needs Humphrey / the accountant).** Corrected understanding (5 June 2026): Shopify does **not** compute VAT on **any** product, DTC or B2B, this store is **not** configured to collect VAT in Shopify (UK shows "Collecting -"; prices are VAT-inclusive and VAT is handled at the accounting layer, not by Shopify). So the earlier "tick charge-tax on the B2B variant" idea is **not** the lever, and we must **not** turn on store-level VAT collection (it would change VAT behaviour on all DTC orders). Implications: (1) B2B orders sync to Parex with no VAT line, so the **Xero invoice's VAT must come from Parex's Sales Tax Code (20% VAT on Income)**, the pilot must confirm Parex stamps the 20% and whether it treats the amount as VAT-inclusive (GBP 59 = 49.17 + 9.83) or exclusive (59 + 11.80); (2) the **B2B pricing model is the open decision**, do B2B prices include VAT (club pays GBP 59) or exclude it (club pays GBP 70.80)? The order page currently displays ex-VAT (70.80) while the store convention is inc-VAT; these must be made consistent and that choice drives the Parex inc/exclusive setting. None of this blocks verifying the Parex sync mechanics. Owner: Rudh + Humphrey / accountant.
