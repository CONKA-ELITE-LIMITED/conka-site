...
- **2026-08-19** — **Skio (Noah) answers captured in docs.** Wrote Noah's 2026-08-18 replies into `skio/migration.md` ("Answers from Skio" section + cutover steps) and the parity plan doc's Phase 3 decision. Headlines: rebills have no `checkout_token` (confirms our skip), attributes persist to renewals only if on the contract (ours are, being cart-level at checkout), native Triple Whale integration for recurring visibility, Skio lifecycle webhooks available, migrated Loop contracts do NOT retain Loop attribution, and Skio writes address/payment back to Shopify (drop the Loop address-mirror at cutover). No more pre-cutover open questions for Skio.
- **2026-08-19** — **Portal auto-login VERIFIED end-to-end (SCRUM-1227).** On the skio-integration preview (after registering that branch alias' callback URI + JS origin in the Headless Customer Account API app), a logged-in customer opening `/account/manage` auto-logs into Skio and lands on their subscription (the cancelled #3879 contract shows), no email-login screen. The `/a/account/shopify-login` fix works. Ticket → For review.
- **2026-08-19** — **Portal auto-login endpoint fixed (SCRUM-1227).** `app/api/auth/skio-portal/route.ts` was building the src on `/a/account/login` (Skio's passwordless email login → "Email does not exist" screen) instead of the auto-login endpoint `/a/account/shopify-login`; corrected per Noah + Skio docs. Host stays `cpv3.skio.com` (v3, confirmed; `storefront-iframe.skio.com` is v2 — Noah's example pasted the v2 host). `totalSpent` kept `0`: display-only, not in the hash, and the Customer Account API exposes no lifetime-spend field (`amountSpent` is Admin-only). CSP `frame-src` unchanged. Behind the flag; branch `fix/skio-portal-autologin-path`. Final auto-login still needs a real Skio test contract for rudhkurup@conka.io (create via 100%-off checkout or the Skio backend); a customer with no Skio contract also falls back to the login form.
- **2026-08-18** — **Attribution + fulfilment parity verified (SCRUM-1223).** Confirmed Skio orders inherit Meta attribution + Synergy fulfilment with no code change — the Loop→Skio swap only touches the variant/plan in `getOfferVariant`; attribution (`buildMetaCartAttributes` → `/api/cart` cart attributes → order `note_attributes`) and the CAPI webhook (`checkout_token` gate) are variant-agnostic. Proven from live prod orders (`#3878`/`#3875`/`#3873`/`#3872` all carry `_fbp`+`_fbc`+`conka_uid`+`_listicle_origin`); the Skio test order `#3879` was a cookie-less/£0-discount session so its `note_attributes` were empty (test artifact, not a defect). Metafield audit of all 6 Skio base variants PASSED (bundlecomposition + weights correct, none `SYNERGYIGNORE`). Written up in `META_PIXEL_AND_CAPI.md` + `skio-attribution-fulfilment-parity.md`. **Next (ops): hand Synergy the 6-SKU→box document, then a live Skio-order routing test** (use a normally-paid order — `#3879` was auto-fulfilled at £0 and won't pull). Open, non-blocking: Events Manager EMQ + first-rebill skip-log (need Rudh's Meta/Vercel access).
