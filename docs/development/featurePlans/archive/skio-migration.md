...
**Post-cutover verification: PASS (2026-09-02).** Audited the first nine Skio-native orders against the Shopify Admin API and the Skio API. Cart attributes (`_fbp`, `_fbc`, `conka_uid`, `_listicle_origin`, `_upsell`) and the line-item `source` property land on every order; the Meta CAPI Purchase fires (confirmed in Vercel prod logs); all nine contracts show the Journey swap completed onto the plain variant at the correct recurring price with no stray discount. Shopify's own `customerJourneySummary` is empty on every order (headless, expected), so `_listicle_origin` plus `conka_uid` plus the Meta identifiers remain the only attribution we have.

**Order tags: Skio stamps one where Loop stamped eight, and nothing automated depended on the difference.** Loop wrote `subscription_order`, `first_subscription_order`, `Billing cycle #N`, `Deliver every 1 MONTH`, `Pay every 1 MONTH`, `Subscription #<id>` and the plan name onto every order; Skio writes `Subscription First Order`. Verified that no code, Klaviyo segment or live retention flow reads order tags, and that `IMPORTSYNERGY` (the only load-bearing tag) is written by Synergy and unaffected by either platform. **Do not ask Skio to replicate the Loop tag set for correctness.** The single real consequence was `docs/analytics/LISTICLE_PERFORMANCE.md` query 7, which defined "renewal" using Loop tags; that rule has been re-keyed onto `checkout_token`, which is platform-independent. Full breakdown, including the separate finding that our own `persona:` tag write has been failing on a missing `write_orders` scope since it shipped, is in **`docs/development/CART_ATTRIBUTES.md`**.
...
- **2026-09-02** - **Post-go-live tag and attribution audit: attribution PASS, tagging has two findings.** Audited the first nine Skio orders end to end (Shopify Admin API, Skio API, Vercel prod logs). Attribution is intact: cart attributes, line-item `source`, Meta CAPI Purchase, and all nine Journey swaps onto the plain variant at the right recurring price. Two tagging findings, **both predating the cutover**: (1) our own `listicle`/`persona:` order tags have never been written, because `addOrderTags()` uses the B2B Invoicing token which has no `write_orders` and `tagsAdd` returns "Access denied" (79 orders with `_listicle_origin` since 10 Aug, zero tagged); (2) `plan_frequency` has never landed on any order, because `SELLING_PLAN_FREQUENCY` still maps only the three retired trial-pack plan ids. Neither loses data: `_listicle_origin` still rides every order. Separately, Skio's thinner tag set broke the renewal test in `LISTICLE_PERFORMANCE.md` query 7, now re-keyed onto `checkout_token`. Both code findings tracked in `docs/TODO.md`; the tag ownership map lives in `docs/development/CART_ATTRIBUTES.md`.
