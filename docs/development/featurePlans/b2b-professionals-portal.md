...
- **Key deviations from the plan:**
  ...
  - **Variant GIDs are server-side constants** in `app/lib/b2bVariants.ts` (`B2B_VARIANTS`), imported by the cart + invoice routes - NOT in `productPricing.ts` / `shopifyProductMapping.ts` as the plan task listed. Originally env vars, moved to constants 8 June 2026: one prod store, not secret, no per-environment variance, so nothing to set in Vercel. (The card route's "not configured" 503 is gone; the invoice route's 503 is now only for a missing Admin API token.)
...
- **Key implementation notes / deviations:**
  ...
  - Reuses the same server-side `B2B_VARIANTS` constants as the card path. Returns a 503 "not available yet" only if the Admin API token is unset (the token is a secret, so it stays in env), same graceful-degradation pattern as `/api/b2b/cart`.
...
**In-code constants, deliberately NOT env (nothing to set in Vercel):** the B2B Shopify variant GIDs (`B2B_VARIANTS` in `app/lib/b2bVariants.ts`) and the Klaviyo list id + alert recipient (`B2B_KLAVIYO` in `app/lib/b2bData.ts`). None are secret, and none vary by environment (one prod Shopify store, one Klaviyo account). Only genuine secrets (`SHOPIFY_ADMIN_API_TOKEN`) and true per-environment values (`NEXT_PUBLIC_SITE_URL`) live in env.

### Outstanding for a fully live portal

- **SCRUM-1056:** ~~create/rename the B2B products + the total-quantity automatic discounts~~ **DONE 8 June 2026** — `B2B Products` collection + two combined-total automatic discounts (£62.40 at 25+, £54.00 at 50+) created and cart-verified inclusive on the headless checkout. The variant GIDs are in-code constants (`app/lib/b2bVariants.ts`), so nothing to set in Vercel for the card path.
