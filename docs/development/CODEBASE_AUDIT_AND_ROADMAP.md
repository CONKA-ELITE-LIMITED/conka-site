...
| # | Issue | What it impacts | Evidence | Severity | Effort |
|---|-------|-----------------|----------|:--------:|:------:|
| Q1 | ~~**Protocol system still fully present** (~3.3k lines)~~ **Resolved.** `app/protocol/`, `app/components/protocol/` and the `app/lib/protocol*.ts` modules are deleted. What remains is the deliberately quarantined commerce layer for existing subscribers: `app/lib/legacy/protocolSubscriptions.ts` plus `ProtocolId` / `PROTOCOL_VARIANTS`. Do not delete that without confirming who still holds a protocol subscription | — | `app/lib/legacy/protocolSubscriptions.ts` | ✅ | — |
...
| Q4 | **Orphaned components left in the tree**, several with their own "delete me" TODOs. **Partly resolved 2026-08-27:** a tree-wide sweep deleted 16 (including `HeroBannerCarousel.tsx`). It was stopped deliberately with 21 still orphaned, because unimported does not mean safe here: the video heroes are parked for revert, `RevolutPayButton` is a payment path, and the account/subscriptions cluster is mid-Skio-migration. The remaining list and the scan method are in `docs/TODO.md` | Confusion, accidental reuse of dead code | `LandingProductSplit.tsx`, `LandingCTA.tsx`, `SynergyChart.tsx`, deprecated `FormulaBenefits*` | 🟡 | S |
| Q5 | ~~**Cognitive test links to deprecated `/protocol/*` routes**~~ **Resolved.** `CognitiveTestRecommendation.tsx` no longer references `/protocol/*`; it points at the live product routes | — | `components/cognitive-test/CognitiveTestRecommendation.tsx` | ✅ | — |
| Q6 | **Significant duplication** — ~13 bespoke hero variants with no shared base; mobile/desktop hero pairs duplicate structure; `/lander` re-implements buy logic (the three funnel forks merged into `/build-your-order` + `app/lib/offerData.ts`, SCRUM-1247) | Changes must be made in many places; drift between variants | `offerData.ts` consumers, `*Hero*` components | 🟠 | L |
