...
| Q6 | **Significant duplication** — ~13 bespoke hero variants with no shared base; mobile/desktop hero pairs duplicate structure; `/lander` re-implements buy logic (the three funnel forks merged into `/build-your-order` + `app/lib/byoData.ts`, SCRUM-1247) | Changes must be made in many places; drift between variants | `byoData.ts` consumers, `*Hero*` components | 🟠 | L |
