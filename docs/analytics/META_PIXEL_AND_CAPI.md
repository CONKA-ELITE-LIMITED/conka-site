...
- **Renewals / scope:** `isWebCheckoutOrder()` only sends Purchase for orders that came through the hosted web checkout — gated on **`checkout_token` present AND `source_name === "web"`**. Subscription rebills (Loop/Skio/native), POS, and API/manual orders are created without a checkout, so they have no `checkout_token` and are skipped. The first subscription order is a real checkout (has a token), so it is kept.
