...
> **`persona:` order tags were removed (2026-09-21).** The `orders/paid` webhook used to call `tagsAdd` to stamp `listicle` + `persona:<name>` (SCRUM-1180), but the live `SHOPIFY_ADMIN_API_TOKEN` (B2B Invoicing app) lacks `write_orders`, so it never once succeeded. The path is now deleted rather than fixed: nothing consumed the tag. Orders still carry the `_listicle_origin` note attribute (that comes from the cart, not the webhook), so attribution is unaffected — filter on the note attribute, **never** the tag. The dated entries below record "0 tagged" counts from when the write was merely broken; they stand as history. See `docs/development/CART_ATTRIBUTES.md`.
