...
**Fix.** `isWebCheckoutOrder()` now gates on **`checkout_token` present** — a genuine hosted-checkout completion (incl. a customer's *first* subscription order) has a `checkout_token`; API-created rebills/POS/manual orders do not, and Loop cannot fake it. `checkout_token` is **domain-independent** and works for our headless storefront (the checkout is the same Shopify-hosted web checkout regardless of storefront domain), so no `shop.conka.io`/domain setup is needed. We deliberately do **not** also require `source_name === "web"`: headless Storefront-API orders can report a non-"web" `source_name`, and requiring it would risk dropping real orders (false negatives — we'd send nothing). Added `checkout_token` + `source_name` to the `ShopifyOrder` interface. Added **temporary verification logging** on both the skip path (logs `sourceName`, `hasCheckoutToken`) and the send path (logs `orderId`, `sourceName`, `value`) — the logged `source_name` lets us see the real value and tighten later only if wanted.

> Note: could not verify `source_name`/`checkout_token` on a live order from the repo — the Admin API token lacks `read_orders` scope (it's the B2B draft-orders token). The temp logging is therefore the verification mechanism: the first day of real webhook payloads confirms the values.
...
**Research backing (2026-06-19, Shopify dev forums).**
- **Headless storefronts return a NUMERIC `source_name`** (e.g. `"16154930"`), NOT `"web"` — confirmed by a Shopify rep. So the originally-shipped `source_name === "web"` requirement would have dropped **every** real order on our headless store and sent Meta nothing. This is why we gate on `checkout_token` only. ([thread](https://community.shopify.dev/t/orders-create-source-name-value-for-headless-storefront-orders/20588))
