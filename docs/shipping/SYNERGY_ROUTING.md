...
- **`Express International DHL` is dormant.** Mapped and DDP, but no Shopify zone points at it. It
  is the likely vehicle for rest-of-world DDP later.
- **Do not retire `Express International`.** Every non-European zone, the Channel Islands rate and
...
- **Mapping:** UK contracts resolve to `Express`, European contracts to `European Delivery`, the
  rest of the world to `Express International`.
- **A re-sync can match nothing.** It picks the rate whose weight band fits, so a contract below a
  zone's minimum (Europe has one) gets no rate. Check the weight before re-syncing an international
  contract.
