 ...
 * AppMeasureSection
 *
 * "We don't ask if CONKA works. We measure it." — the app proof section.
 * Ported from the lander (app/lander/sections/Measure) into our patterns:
 * Tailwind + the shared useInView hook + motion-safe inline transitions, no
 * CSS Module. Two scroll-triggered effects on first view:
 *   1. SVG cognitive-score line draws (stroke-dash, length measured at mount).
 *   2. The score counts up 72 -> 89 via requestAnimationFrame.
 * Both respect prefers-reduced-motion (line snaps drawn, score snaps to final).
 *
 * Renders a dark card so it sits in the listicle's dark-card-on-light rhythm
 * (like statsBand / bridge); the renderer owns the surrounding light section.
 * ========================================================================== */
...
export default function AppMeasureSection() {
  ...
  // Measure the line once so the draw matches its exact length.
  useEffect(() => {
  ...
  // Count the score up on first view (snap to final under reduced motion).
  useEffect(() => {
  ...
  return (
    <div
      ref={ref}
      className="overflow-hidden rounded-3xl bg-[#101a33] px-5 py-12 text-white md:px-10 md:py-16"
    >
      {/* Header */}
      ...
      {/* Cognitive-score app card */}
      <div className="mx-auto mb-8 max-w-[34rem] rounded-[22px] border border-white/12 bg-[#18233f] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.35)]">
        ...
        >
          <defs>
            <linearGradient id="meas-grad" x1="0" y1="0" x2="1" y2="0">
      ...
      {/* 1-2-3 steps */}
      <div className="relative mx-auto mb-7 flex max-w-[40rem] justify-between gap-2">
        <div
